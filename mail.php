...
if (!isset($_GET['czytaj']) && !isset($_GET['opcja']) /* && !isset($_GET['del'])*/)
{
	...
	else close ('</table><br /><div style="text-align: center;">Nie masz żadnych wiadomości!</div>');
	echo '</table>';
}
...
{  
    if (!isset($_GET['zobacz']))
	{
		...
		else close ('</table><br /><div style="text-align: center;">Brak wysłanych wiadomości!</div>');
		echo '</table>';
	}
	    else
	{ 
