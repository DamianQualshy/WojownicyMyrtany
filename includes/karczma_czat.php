...
{    
	...
	echo $message;
	echo '<div id="karczma_czat">';
	
	$karczma_chat = mysqli_query($GLOBALS['db'], "SELECT * FROM `chat` ORDER BY `id` DESC LIMIT 10");
    ...
    if(mysqli_num_rows($karczma_chat) > 0) 
    {
        ...
        { 
			...
			if ($rank == 'Admin') $del = '<a style="margin-right: 2px; margin-top: -2px; float: right;" href="karczma.php?del='.$i -> id.'"><font style="color: #ff0000;">X</font></a>';
			else $del = '';
            if($i -> autor == $user)
			{
			$del = '<a style="margin-right: 2px; margin-top: -2px; float: right;" href="karczma.php?del='.$i -> id.'"><font style="color: #ff0000;">X</font></a>';
			echo '<div title="['.$i -> data.'] ['.$i -> czas.']" style="word-wrap: break-word; border-radius: 16px 0px 16px 16px; padding: 3px; margin-bottom: 2px; background: #505050"><b> <div style="font-family: Englebert; color: #E9E9E9; margin-left: 8px; margin-top: 4px; margin-bottom: 4px;">'.$del.' <a style="color: white;" href="view.php?name='.$i -> autor.'">'.$i -> autor.'</a></b>: <br>'.$i -> text.'</div></div>';
			}
   	        else echo '<div title="['.$i -> data.'] ['.$i -> czas.']" style="word-wrap: break-word; border-radius: 16px 0px 16px 16px; padding: 3px; margin-bottom: 2px; background: #2B2B2B"><b> <div style="font-family: Englebert; color: #E9E9E9; margin-left: 8px; margin-top: 4px; margin-bottom: 4px;">'.$del.' <a style="color: white;" href="view.php?name='.$i -> autor.'">'.$i -> autor.'</a></b>: <br>'.$i -> text.'</div></div>';
        }
    }
    else echo '<br />Nie ma żadnych wypowiedzi w czacie<br />';
    ...
    echo "<input class=\"formul\" type=\"text\" onInput=\"licz(this,200)\" name=\"text\" style=\"background: url(../images/input_dlugi.png); width: 284px; padding-left: 8px; padding-right: 8px;\" value=\"Tekst\" onfocus=\"if (this.value == 'Tekst') this.value = '';\" onblur=\"if (this.value == '') this.value = 'Tekst';\" /> ";
	echo '</td>';
	echo '<td>';
	...
	echo '<tr>';
	echo '<td>';
    echo 'Pozostało znaków: <input id="wskaz" value="200" readonly="readonly" style="width: 40px; font-size: 18px; color: whitesmoke; background: none; border: none; font-weight: bold;" type="text" />';
	echo '</td>';
	echo '<td>';
	echo '</td>';
	echo '</tr>';
...
if (isset($_GET['send']))
{
    if (strlen($_POST['text']) <= 200) 
	{
        ...
        { 
            ...
            send($user -> get['rank'], '<span style="color:green;">Dodano wypowiedź</span><br /><br />', $user -> get['login']);
        }
	    else send($user -> get['rank'], '<span style="color:red;">Musisz coś wpisać!</span><br /><br />', $user -> get['login']);
    }
	else send($user -> get['rank'], '<span style="color:red;">Zbyt długa wypowiedź! Maksymalnie możesz wpisać 400 znaków!</span><br /><br />', $user -> get['login']);
}
