	...
	<div class="postbreak"></div>
		...
		if (isset($_POST['trening']))
		{
			switch ($_POST['trening'])
			{
				...
				default: $zmienna1 = 'Twoja zręczność wzrosła'; $zmienna2 = 'szybkosc'; break;
			}
			if ($user -> get['pkt'] >= $zmienna4)
			{
				echo '<p><span class="komunikat" style="color:green; font-size: 20px;">'.$zmienna1.' o '.$zmienna3.' wartości za '.$zmienna4.' PN.</span></p><br />';
				mysqli_query($GLOBALS['db'], 'UPDATE `konta` SET `'.$zmienna2.'`=`'.$zmienna2.'`+'.$zmienna3.', `pkt`=`pkt`-'.$zmienna4.' WHERE `id`='.$user -> get['id']);
			}
			else echo '<span class="komunikat" style="color:red; font-size: 20px;">Wróć, kiedy nabierzesz więcej doświadczenia.</span><br /><br />';
		}
