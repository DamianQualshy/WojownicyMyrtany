			...
			while ($zgloszenie = mysqli_fetch_array($a1))
			{
				...
				echo '
					  <tr style="height: 40px; background: '.$przeczytane.'">
						...
						<th style="width: 160px; padding-left: 5px; padding-right: 5px;">
						...
						echo ' <a style="color: #8181f8" href="zgloszenia.php?usun='.$zgloszenie['id'].'">Usuń</a>
						</th>
					  </tr>';
			}
			echo '</table>';
...
if (isset($_GET['usun']))
{
	if (preg_match("/^[0-9]*$/", $_GET['usun']) && !empty($_GET['usun']))
	{
	    ...
	    if ($checkq == true)
		{
			...
			if (isset($_GET['arch']))
			{
				header('Location: zgloszenia.php?archiwum');
			} else header('Location: zgloszenia.php');
		}
	}
}
